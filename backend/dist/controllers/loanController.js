...
exports.getAdminStats = exports.recordRepayment = exports.getActiveLoans = exports.disburseLoan = exports.getSanctionedLoans = exports.reviewLoan = exports.getAppliedLoans = exports.getSalesLeads = exports.getBorrowerLoans = exports.applyForLoan = void 0;
...
const calculateLoanMath = (amount, tenureDays) => {
...
const applyForLoan = async (req, res) => {
    try {
        ...
        if (req.file) {
            documentUrl = await (0, cloudinary_1.uploadSalarySlip)(req.file.path, req.file.filename);
        }
        ...
        const initialStatus = breResult.eligible ? 'Applied' : 'BRE_Rejected';
        const rejectionReason = breResult.eligible ? undefined : breResult.reason;
        const newLoan = new Loan_1.Loan({
            borrowerId: new mongoose_1.default.Types.ObjectId(req.user.id),
            personalDetails,
...
const getBorrowerLoans = async (req, res) => {
    try {
        ...
        const loans = await Loan_1.Loan.find({ borrowerId: req.user.id }).sort({ createdAt: -1 });
...
exports.getBorrowerLoans = getBorrowerLoans;
const getSalesLeads = async (req, res) => {
    try {
        const borrowers = await User_1.User.find({ role: 'Borrower' }).select('-passwordHash').sort({ createdAt: -1 });
        const leads = [];
        for (const borrower of borrowers) {
            const loan = await Loan_1.Loan.findOne({ borrowerId: borrower._id }).sort({ createdAt: -1 });
...
const getAppliedLoans = async (req, res) => {
    try {
        const loans = await Loan_1.Loan.find({ status: 'Applied' })
...
const getActiveLoans = async (req, res) => {
    try {
        const loans = await Loan_1.Loan.find({ status: { $in: ['Disbursed', 'Closed'] } })
            .populate('borrowerId', 'name email')
            .sort({ updatedAt: -1 });
        return res.json(loans);
    }
...
const getAdminStats = async (req, res) => {
    try {
        ...
        const pendingLoans = await Loan_1.Loan.countDocuments({ status: 'Applied' });
        const sanctionedLoans = await Loan_1.Loan.countDocuments({ status: 'Sanctioned' });
        const closedLoans = await Loan_1.Loan.countDocuments({ status: 'Closed' });
        const breRejectedLoans = await Loan_1.Loan.countDocuments({ status: 'BRE_Rejected' });
        ...
        const totalBorrowers = await User_1.User.countDocuments({ role: 'Borrower' });
        return res.json({
            counts: {
                totalLoans,
                activeLoans,
